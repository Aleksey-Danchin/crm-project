...
function init () {
    ...
    document
        .querySelector('[data-mode-panel]')
        .addEventListener('click', event => {
            event.preventDefault()

            const liElement = event.target.closest('li')

            if (liElement && liElement.hasAttribute('data-status')) {
                const status = liElement.getAttribute('data-status')

                document.querySelector('[data-filter-status]').value = status
                setState({ status })
            }
        })
...
function updatePageNumbers (pages, pageNumber) {
    ...
    const nextButton = document.querySelector('[data-pagination-next]').closest('li')
    const prevButton = document.querySelector('[data-pagination-prev]').closest('li')
